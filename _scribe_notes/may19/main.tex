...
\begin{tikzpicture}[
  font=\small,
  layer/.style={draw, rounded corners=2pt, align=center, minimum height=0.92cm, text width=3.0cm},
  meta/.style={draw, rounded corners=2pt, align=center, font=\footnotesize, text width=6.8cm, fill=white},
  arr/.style={-{Latex[length=2mm]}, thick},
...
\node[layer, fill=lightblue] (sched) at (3.55,0) {serving engine\\scheduling, batching};
\node[layer, fill=lightorange] (mem) at (7.10,0) {KV memory manager\\allocation, reuse};
\node[layer, fill=lightgreen] (kern) at (10.65,0) {kernel library\\ragged attention, MLP};
\node[meta] (meta) at (7.10,-1.45) {metadata passed to kernels:\\sequence lengths, cache-block IDs, prefill/decode phase};
\draw[arr] (api) -- (sched);
\draw[arr] (sched) -- (mem);
\draw[arr] (mem) -- (kern);
\draw[metaarr] (sched.south) |- (meta.west);
\draw[metaarr] (mem.south) -- (meta.north);
\draw[metaarr] (meta.east) -| (kern.south);
\end{tikzpicture}
}
...
\path[use as bounding box] (-0.65,-1.18) rectangle (11.55,3.35);
% Left plot: memory capacity
\node[font=\bfseries] at (2.45,3.15) {memory use};
\draw[axis] (0,0) -- (5.0,0) node[anchor=west] {batch size};
\draw[axis] (0,0) -- (0,2.9) node[above] {GB};
\draw[dashedline, red!70!black] (0,2.55) -- (4.75,2.55) node[right, black] {capacity};
\draw[curve, cache] (0,1.45) -- (4.45,2.72);
\draw[curve, decode] (0,1.45) -- (4.45,2.18);
\node[point] at (1.25,1.81) {};
\node[point] at (3.85,2.07) {};
\node[anchor=east] at (-0.08,1.45) {weights};
\node[align=center, font=\footnotesize] at (2.3,1.08) {better KV layout\\flatter growth};
\node[align=center, font=\footnotesize] at (2.95,2.70) {naive allocation\\hits wall early};
...
\node[align=center, font=\footnotesize, anchor=north] at (1.25,-0.20) {capacity\\without good\\KV mgmt.};
\node[align=center, font=\footnotesize, anchor=north] at (3.85,-0.20) {capacity\\with better\\KV mgmt.};
\node[font=\footnotesize, align=center] at (2.7,1.05) {more resident requests\\keep more SMs busy};
\end{scope}
\end{tikzpicture}
}
